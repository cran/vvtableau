#' Get project from Tableau Server
#'
...
get_server_projects <- function(base_url, api_version = 3.4, site_id, token, page_size = 100, include_metadata = FALSE) {
  ...
  df <- as.data.frame(jsonlite::fromJSON(jsonResponseText), check.names = FALSE) %>%
    dplyr::rename_with(~ stringr::str_remove(., "projects.project."), dplyr::everything())
