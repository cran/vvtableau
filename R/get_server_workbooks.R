...
get_server_workbooks <- function(base_url, api_version = 3.4, site_id, token, page_size = 100, include_metadata = FALSE) {
  ...
  df <- as.data.frame(jsonlite::fromJSON(jsonResponseText), check.names = FALSE) %>%
    ...
    dplyr::rename_with(~ stringr::str_remove(., "workbooks.workbook."), dplyr::everything())
